...
io.on('connection', (socket) => {
  ...
  socket.on('BE-join-room', ({ roomId, userName, role }) => {
    ...
    if (assignedRole === 'viewer') {
      ...
      const currentViewers = roomClients.filter(
        (clientId) => socketList[clientId] && socketList[clientId].role === 'viewer'
      );
  ...
  // ✅ FIX: Handle broadcaster request to get existing viewers
  socket.on('BE-request-existing-viewers', ({ roomId }) => {
    const clients = Array.from(io.sockets.adapter.rooms.get(roomId) || []);
    const viewerIds = clients.filter((id) => socketList[id]?.role === 'viewer');
    socket.emit('FE-existing-viewers', viewerIds);
  });
